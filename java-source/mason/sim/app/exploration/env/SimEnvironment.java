...
//	private BrokerAgent broker;
    ...
    public SimEnvironment(SimState state, int width, int height, int nAgents) {
        ...
        this.explorers = new Vector<>(nAgents);
        network = Network.getInstance();
        network.init();
        this.setup(state);

        try {
            writer = new PrintWriter(new File(String.format("stats/stats-%d.csv",runNumber++)));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private void addExplorer(SimState state, Int2D loc) {
        ExplorerAgent explorer = new ExplorerAgent(explorerCount++, loc, world.getWidth(), world.getHeight());
...
//		explorer.broker = broker;
    ...
    private void buildDonutMap(SimState state) {
        ...
        for (int j = 0; j < num_inner; j++) {
            do {
                loc = new Int2D(state.random.nextInt(inner_width) + inner_x, state.random.nextInt(inner_height) + inner_y);
            }
            while (occupied[loc.x][loc.y] != null);
    ...
    private void buildStructuredMap(SimState state) {
        ...
        for (int j = 0; j < num_instances; j++) {
            do {
                loc = new Int2D(state.random.nextInt(world.getWidth()), state.random.nextInt(height_separation - sep / 2));
            }
            while (occupied[loc.x][loc.y] != null);
        ...
        for (int j = 0; j < num_instances; j++) {
            do {
                loc = new Int2D(state.random.nextInt(world.getWidth()), state.random.nextInt(30) + (height_separation - 30 / 2));
            }
            while (occupied[loc.x][loc.y] != null);
        ...
        for (int j = 0; j < num_instances; j++) {
            do {
                loc = new Int2D(state.random.nextInt(world.getWidth()), state.random.nextInt(height_separation - sep) + (height_separation + sep / 2));
            }
            while (occupied[loc.x][loc.y] != null);
        ...
        for (int j = 0; j < num_instances; j++) {
            do {
                loc = new Int2D(state.random.nextInt(world.getWidth()), state.random.nextInt(30) + (2 * height_separation - 30 / 2));
            }
            while (occupied[loc.x][loc.y] != null);
        ...
        for (int j = 0; j < num_instances; j++) {
            do {
                loc = new Int2D(state.random.nextInt(width_separation - sep / 2), state.random.nextInt(height_separation - sep / 2) + (2 * height_separation + sep / 2));
            }
            while (occupied[loc.x][loc.y] != null);
        ...
        for (int j = 0; j < num_instances; j++) {
            do {
                loc = new Int2D(state.random.nextInt(width_separation) + (width_separation), state.random.nextInt(height_separation - sep / 2) + (2 * height_separation + sep / 2));
            }
            while (occupied[loc.x][loc.y] != null);
        ...
        for (int j = 0; j < num_instances; j++) {
            do {
                loc = new Int2D(state.random.nextInt(width_separation - sep / 2) + (2 * width_separation + sep / 2), state.random.nextInt(height_separation - sep / 2) + (2 * height_separation + sep / 2));
            }
            while (occupied[loc.x][loc.y] != null);
    ...
    private void addObject(Class c, Int2D loc) {
        ...
        try {
            ...
            System.err.println("Oops. See addObject.");
            return;
        }
    ...
    private void printStats() {
        ...
        for (int i = 0; i < world.getWidth(); i++) {
            for (int j = 0; j < world.getHeight(); j++) {
                ...
                Class identified = network.getIdentifiedObject(i, j);
...
/*
        ...
        System.err.println("-------------------------");
        System.err.println("Number of messages: " + network.getNumberOfMessages());
        System.err.println("-------------------------");
*/
        writer.write("" + step + " , " + ((double) objsSeen / (double) nObjs) * 100.0 + " , " + ((double) nErrors / (double) objsSeen) * 100.0 + " , " + network.getNumberOfMessages()+"\n");
    }
    ...
    /**
     * Return the object at the location provided as parameter
     *
     * @param loc
     * @return
     */
