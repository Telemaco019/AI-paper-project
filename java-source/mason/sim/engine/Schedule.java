...
/**
   ...
   <p>You can init out the entire Schedule by calling reset(), including about-to-be executed Steppables in the
...
public class Schedule implements java.io.Serializable
    ...
    public synchronized boolean step(final SimState state)
        ...
        int topSubstep = 0;  // we set this as a hack to avoid having to init all the substeps each time until the very end
        ...
        synchronized(lock)
            ...
            // 4. Then we init the substeps bag, but we don't let them GC yet
            // 5. Last, out of the while-loop, we init the substeps bag "for real", allowing them to GC
            while(true)
                {
                ...
                // dump
                if (topSubstep < substeps.numObjs) topSubstep = substeps.numObjs;  // remember index of largest substep since we're violating init()
                currentSteps.addAll(substeps);
                substeps.numObjs = 0;  // temporarily init
        ...
        // now finally init out the substeps for real
        substeps.numObjs = topSubstep;
        substeps.clear();  // init for real so everything can GC
        ...
        try
            ...
            {
            // reuse currentSteps -- all objects should have been released to gc already, no need to call init()
