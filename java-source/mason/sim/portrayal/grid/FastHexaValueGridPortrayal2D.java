        ...
        if (graphics!=null && shouldBuffer(graphics))
            ...
            synchronized(this)
                ...
                if (buffer==null || buffer.getWidth() != maxX || buffer.getHeight() != (2*maxY+1))
                    ...
                    // new BufferedImage(...)
                    if (buffer != null) buffer.flush();  // in case Java forgets to init memory -- bug in OS X
...
//                              _raster.setDataElements(x,2*y+2,load);
        ...
        // finally, init dirty flag if we've just drawn (don't init if we're doing hit testing)
